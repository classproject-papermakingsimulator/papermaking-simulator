Shader "Water/WaterPro_Heightmap" 
{
	/* 
	...
	Properties 
	{
		...
		_EdgeFade ("Edge fade", Range(0,50)) = 0.5
	...
	SubShader 
	{
		...
		////// Uniform user variable definition
			...
			uniform sampler2D _HeightTex;				// The terrain height map
			uniform float4 _HeightTex_ST;				// Scale and offset of the height map
		...
		////// Shader functions
			...
			{
				Vert2FragPro output;
				output = HandleVertexInput(vertIn, _MainTex_ST, _NormalTex_ST, _HeightTex_ST, _UVOffset, _FoamScale);
				return(output);
			}
			...
			float4 frag(Vert2FragPro fragIn) : SV_Target
			{
				...
				// Depth fog
				float depthLevel = GetDepthValue(fragIn.posScreen, _LastCameraDepthTexture, _EdgeFade);
				float waterLevel = GetWaterLevel(_HeightTex, fragIn.uv_Environment, _ShallowDeepFade, _ShallowDepth);
				...
				// Foam
				float3 foam = GetFoamHeightMap(_HeightTex, _FoamTex, fragIn.uv_Environment, _FoamOffset, _FoamFade);
